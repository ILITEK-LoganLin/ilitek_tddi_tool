...
static void dump_node_type_buffer(s32 *node_ptr, char *name)
{
	int i;
	char dump_log[R_BUFF_SIZE] = {0};
	...
	if (isspace_t((int)(unsigned char)*s) == 0) {
		while ((pToken = strsep(&s, ",")) != NULL) {
			...
			if (ret == 0)
				...
				ILI_INFO("convert string too long, ret = %d", ret);*/
			buf[conut] = strtol(pToken, NULL, base);
			conut++;
...
int ili_mp_ini_parser(const char *path)
{
	int i, ret = 0, fsize = 0;
	char *tmp = NULL;
	FILE *f = NULL;
	...
	f = fopen(path, "r");
	if (ERR_ALLOC_MEM(f)) {
		ILI_ERR("Failed to open the file at %s\n",path);
		ret = -ENOENT;
		goto out;
	}
...
static int precommd(u8 *cmd, int cmdlen, u8 *precmd, int precmdlen, u8 *rdata, u32 rlen)
{
	...
	if (ilits.wrapper(buf, 2, NULL, 0) < 0) {
	...
	if (ilits.wrapper(precmd, precmdlen, NULL, 0) < 0) {
	...
	if (ilits.wrapper(cmd, cmdlen, ilits.rbuf, rlen) < 0) {
		ILI_ERR("Failed to get cdc data\n");
		return -EMP_GET_CDC;
	}

	memcpy((u8 *)rdata, &ilits.rbuf[READ_OFFSET], rlen);
...
static int allnode_key_cdc_data(int index)
{
	...
	ret =  ilits.wrapper(cmd, sizeof(cmd), ilits.rbuf, len);
	if (ret < 0) {
		ILI_ERR(" fail to get cdc data\n");
		goto out;
	}
	memcpy(ori, &ilits.rbuf[MP_CDC_READ_OFFSET], len);
...
static int allnode_open_cdc_data(int mode, int *buf, int index)
{
	...
	if (tItems[index].pre_cmd_en) {
		...
		ili_dump_data(precmd, 8, precmd_len, 32, "PreCommand");
	...
	ili_dump_data(ori, 8, len, 32, "Open SP CDC original");
...
static int allnode_peak_to_peak_cdc_data(int index)
{
	...
	if (frm_buf == NULL) {
		frm_buf = (s32 **)malloc(tItems[index].bch_mrk_frm_num * sizeof(s32 *));
		if (ERR_ALLOC_MEM(frm_buf)) {
			ILI_ERR("Failed to allocate frm_buf mem\n");
			ret = -EMP_NOMEM;
			goto out;
		}
		for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
			frm_buf[i] = (s32 *)malloc(core_mp.frame_len * sizeof(s32));
			if (ERR_ALLOC_MEM(frm_buf)) {
				ILI_ERR("Failed to allocate frm_buf[%d] mem\n", i);
				ret = -EMP_NOMEM;
				goto out;
			}
		}
...
static int allnode_mutual_cdc_data(int index)
{
	...
	for (i = 0; i < core_mp.frame_len; i++) {
		if (ipio_strcmp(tItems[index].desp, "calibration data(dac)") == 0) {
			...
			if (ipio_strcmp(tItems[index].desp, "raw data_vmd_noise(no bk)") == 0 ||
			    ipio_strcmp(tItems[index].desp, "raw data(no bk)") == 0 ||
	...
	if (tItems[index].catalog == TX_RX_DELTA) {
		...
		if (tItems[index].spec_option == BENCHMARK) {
			...
			if (tItems[index].bch_mrk_min == NULL) {
				tItems[index].bch_mrk_min = (s32 **)malloc(tItems[index].bch_mrk_frm_num * sizeof(s32 *));
				...
				for (i = 0; i < tItems[index].bch_mrk_frm_num; i++) {
					tItems[index].bch_mrk_min[i] = (s32 *)malloc(core_mp.frame_len * sizeof(s32));
...
static int mp_show_result()
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		if (tItems[i].run) {
			ILI_DBG("tItems[%d] result = %d\n",i,tItems[i].item_result);
	...
	ILI_INFO("pass_item_count = %d, core_mp.final_result = %d\n", pass_item_count, core_mp.final_result);
	...
	if(ilitsmp->csv_fp != -1) {
		...
		ILI_ERR("CSV open point is NULL!\n");
		ret = -EMP_NOMEM;
		goto fail_open;
	}
...
void ili_mp_init_item(void)
{
	...
	core_mp.protocol_ver = ilits.chip.potocal_ver;
	core_mp.core_ver = ilits.chip.core_ver;
	core_mp.cdc_len = ilitsmp->protocol->cdc_len;
	core_mp.no_bk_shift = ilits.chip.no_bk_shift;
	core_mp.xch_len = ilitsmp->tp_info->xch_num;
	...
	ILI_INFO("CHIP = 0x%8X\n", core_mp.chip_pid);
	ILI_INFO("Firmware version = %8X\n", core_mp.fw_ver);
	ILI_INFO("Protocol version = %8X\n", core_mp.protocol_ver);
	ILI_INFO("Core version = %8X\n", core_mp.core_ver);
...
static void ilitek_tddi_fw_mp_precmd_support(void)
{
	...
	ILI_INFO("ilitek_tddi_fw_mp_precmd_support\n");
	...
	ret = ilits.wrapper(cmd, 2, ilits.rbuf, 3);
	if (ret < 0)
		ILI_ERR("check FW pre command failed\n");
	buftmp = ilits.rbuf[READ_OFFSET + 2];
	if (buftmp == 0xAA)
				...
				if (tItems[i].bch_mrk_max != NULL) {
					for (j = 0; j < tItems[i].bch_mrk_frm_num; j++)
						if (tItems[i].bch_mrk_max[j] != NULL) {
							ipio_kfree((void **)&tItems[i].bch_mrk_max[j]);
							tItems[i].bch_mrk_max[j] = NULL;
						}
					ipio_kfree((void **)&tItems[i].bch_mrk_max);
					tItems[i].bch_mrk_max = NULL;
				}
				if (tItems[i].bch_mrk_min != NULL) {
					for (j = 0; j < tItems[i].bch_mrk_frm_num; j++)
						if (tItems[i].bch_mrk_min[j] != NULL) {
							ipio_kfree((void **)&tItems[i].bch_mrk_min[j]);
							tItems[i].bch_mrk_min[j] = NULL;
						}
					ipio_kfree((void **)&tItems[i].bch_mrk_min);
					tItems[i].bch_mrk_min = NULL;
				}
			}
...
void ili_mp_copy_result(char *buf, size_t size)
{
	int i, run = 0;

	if (size < MP_TEST_ITEM) {
		ILI_ERR("The size given from caller is less than item\n");
		return;
	}

	for (i = 0; i < MP_TEST_ITEM - 1; i++) {
		buf[i] = MP_ITEM_NA;
		if (tItems[i].run) {
			if (tItems[i].item_result == MP_DATA_FAIL)
				...
				buf[i] = MP_ITEM_PASS;
			run++;
			ILI_INFO("result[%d] = %d, name=%s\n", i, buf[i], tItems[i].desp);
		}
	}
}
...
void show_commom_mp_result(void)
{
	...
	for (i = 0; i < MP_TEST_ITEM; i++) {
		if (tItems[i].run) {
			if (tItems[i].item_result == MP_DATA_FAIL) {
				// ILI_INFO("[%s] = FAIL\n", tItems[i].desp);
				output_len += snprintf(testresult + output_len, (OUTPUT_SIZE - output_len), "[%s] = FAIL\n", tItems[i].desp);
			} else {
				// ILI_INFO("[%s] = PASS\n", tItems[i].desp);
				output_len += snprintf(testresult + output_len, (OUTPUT_SIZE - output_len), "[%s] = PASS\n", tItems[i].desp);
			}
		}
	}
	output_len += snprintf(testresult + output_len, (OUTPUT_SIZE - output_len), " MP TEST %s\n", (core_mp.final_result == MP_DATA_PASS) ? "PASS" : "NG");
	printf("%s", testresult);
	free(testresult);
}
	...
	for (i = 0; i < MAX_SECTION_NUM; i++) {
		for (j = 0; j < MP_TEST_ITEM; j++) {
			...
			if (tItems[j].run != 1 || tItems[j].lcm != lcm_on) {
			    tItems[j].run = DISABLE;
				continue;
			// ignore pin test ( int and rst )
			} else if (ipio_strcmp(tItems[j].desp, "pin test ( int and rst )") == 0) {
				ILI_INFO("ignore pin test ( int and rst )\n");
				tItems[j].run = DISABLE;
				continue;
			}
...
int ili_mp_test_main(bool lcm_on)
{
	int i, ret = 0;
	char str[128] = {0}, ver[128] = {0};

	ILI_DBG("Check X/Y Channel Number\n");
	if (ilitsmp->tp_info->xch_num <= 0 || ilitsmp->tp_info->ych_num <= 0) {
		ILI_ERR("Invalid frame length (%d, %d)\n", ilitsmp->tp_info->xch_num, ilitsmp->tp_info->ych_num);
		ret = MP_TP_INFO_INVALID_FAIL;
		goto out;
	}

	ILI_DBG("Init MP Item\n");

	ili_mp_init_item();

	ILI_DBG("Check Protocol Version\n");
	...
	if ((ili_str2hex(ver)) != (core_mp.protocol_ver >> 8)) {
		...
		ret = MP_PROTOCOL_VERSION_INVALID_FAIL;
		goto out;
	}

	ILI_DBG("Switch to Test Mode\n");
	// ret = ili_core_switch_tp_mode(P5_X_FW_TEST_MODE);
	ret = ili_hid_switch_tp_mode(P5_X_FW_TEST_MODE);
	if (ret < 0) {
		ILI_ERR("Switch to test mode failed\n");
		ret = MP_SWITCH_MODE_FAIL;
		goto out;
	}
